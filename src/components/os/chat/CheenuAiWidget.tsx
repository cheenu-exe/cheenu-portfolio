...
interface CheenuAiWidgetProps {
  onNavigate?: (view: View) => void;
  onOpenPdf?: () => void;
}

const SUGGESTIONS = [
  "Show me Cheenu's resume",
  "What has Cheenu built?",
  "What's her AI stack?",
...
export function CheenuAiWidget({ onNavigate, onOpenPdf }: CheenuAiWidgetProps) {
    ...
    if (isBlocked(text)) {
      ...
      setMessages((m) => [
        ...
        { role: "user", content: text },
        {
          role: "assistant",
          content:
            "I'm MYSTI — I'm here to tell you about Cheenu's work and skills. I can't help with that request.",
        },
      ]);
      setInput("");
      return;
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) {
          ...
          if (errorType === "quota") {
            ...
            errorMsg =
              "MYSTI has hit its daily AI limit. It resets at midnight — come back tomorrow!";
          } else {
            toast.error("slow down — rate limited");
            errorMsg = "MYSTI is thinking too fast! Wait a moment and try again.";
          }
          ...
          toast.error("MYSTI is offline");
          errorMsg = "MYSTI is currently unreachable. Please try again later.";
        }
  ...
  return (
    <>
      <button
        ...
        aria-label={open ? "Close MYSTI chat assistant" : "Open MYSTI chat assistant"}
        aria-expanded={open}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            role="dialog"
            aria-label="MYSTI chat assistant"
            aria-modal="true"
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-[9px] font-mono text-white/40">
                    MYSTI Neural Engine · Active Copilot
                  </div>
                </div>
              </div>
              <button
            ...
            >
              {messages.length === 0 && (
                <div className="space-y-3">
                  <p className="text-xs text-white/70 leading-relaxed font-sans">
                    Hi! I'm <strong className="text-primary font-semibold">MYSTI</strong>, Cheenu's
                    OS Copilot. Ask me about her projects, skills, or resume, or click an action
                    below:
                  </p>
              ...
              {loading && (
                <div className="flex items-center gap-2 font-mono text-xs text-cyan-400/80 bg-cyan-950/20 p-2.5 rounded-lg border border-cyan-500/20">
                  ...
                  <span>MYSTI copilot is thinking...</span>
                </div>
              )}
            </div>
